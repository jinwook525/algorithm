...
class Stack4 {
	...
	public Stack4(List<Point> data, int capacity, int top) {
		super();
	...
	public boolean push(Point x) throws OverflowGenericStackException {
		if (isFull()) {
			throw new OverflowGenericStackException("full");
		}
		top++;
		return data.add(x);
...
public class train_QueenEight_구현실습과제 {
	static void EightQueen(int data[][]) {
...
//배열 d에서 행 cx, 열 cy에 퀸을 남서, 북동 대각선으로 배치할 수 있는지 조사
	public static boolean checkDiagSW(int[][] d, int cx, int cy) { // x++, y-- or x--, y++ where 0<= x,y <= 7
		...
		for (int x = cx, y = cy; 0 <= x && y < d[0].length; x--, y++) {
			if (d[x][y] == 1)
				return false;
